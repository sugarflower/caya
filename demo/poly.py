...
class myApp(CAYA):
	...
	def loop(self):
		...
		for l in self.link:
			...
			if self.cl(p2):
				cl = math.floor(254 * self.light(v))
				filled_polygon(self.getSurface(), p2, Color(cl,cl,cl))

		if self.isKeyDownCode(119):
			self.rx = (self.rx - 0.5) % 360
		if self.isKeyDownCode(115):
			self.rx = (self.rx + 0.5) % 360
		if self.isKeyDownCode(100):
			self.ry = (self.ry - 0.5) % 360
		if self.isKeyDownCode(97):
			self.ry = (self.ry + 0.5) % 360
